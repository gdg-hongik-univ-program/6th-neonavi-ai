...
> **먼저 받아야 할 것 2가지** (레포에 없음 — 안중현에게 요청)
> 1. **API 키 2개** (카카오 REST 키 / JavaScript 키)
> 2. **데이터 묶음** `neonavi-data-essential.zip` (17MB — 학습된 모델 + 도로·신호 전처리 캐시)
>
> 키는 공개 저장소에 올리면 안 되므로 레포에 넣지 않았다.
...
## 2. 키 파일 만들기
받은 키를 아래 두 파일에 넣는다. (`.env.example` 참고, 두 파일 모두 git에 올라가지 않음)

**`.env`** — 레포 최상단
```
KAKAO_REST_API_KEY=받은_REST_키
```

**`frontend/.env`**
```
REACT_APP_KAKAO_JS_KEY=받은_JavaScript_키
```

> **Windows 주의**: 메모장으로 저장하면 이름이 `.env.txt` 가 되기 쉽다.
> 저장할 때 파일 형식을 **"모든 파일"** 로 바꾸고 이름을 `.env` 로 적을 것.
> (VS Code 로 만들면 이 문제가 없다)

## 3. 데이터 풀기
받은 zip을 **레포 최상단에서** 푼다. `ai/data/` 아래로 알아서 들어간다.

- macOS/Linux: `unzip neonavi-data-essential.zip`
- Windows: 파일 탐색기에서 zip 우클릭 → **"압축 풀기"** → 대상 폴더를 **레포 최상단**(`6th-neonavi-ai`)으로 지정

확인 — 아래 두 경로가 있어야 한다:
```
ai/data/model_a.pt
ai/data/public/derived/
```
...
| 증상 | 원인 / 해결 |
|---|---|
| 지도가 회색이고 "지도를 표시할 수 없습니다" | `frontend/.env` 의 JS 키 확인 → **넣은 뒤 `npm start` 재시작** (.env는 시작할 때만 읽음) |
| "경로를 불러오지 못했습니다" | 백엔드가 떠 있는지 확인 (터미널 1) |
| "추천 모델을 불러오지 못했습니다" | 3단계 데이터 압축 해제 확인 (`ai/data/model_a.pt` 있는지) |
| `KAKAO_REST_API_KEY 없음` | 레포 최상단 `.env` 확인. Windows 면 파일명이 `.env.txt` 가 아닌지도 확인 |
